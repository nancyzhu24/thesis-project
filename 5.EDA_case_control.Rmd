---
title: "Case control-exclude prevalent cases"
author: "Nancy"
date: "April 30, 2018"
output: 
  html_document:
...
1.having an AS diagnostic code or a surgical valve replacement after cohort entry (inpatient data)

at least **365 days** after cohort entry
...
ggplot(ramq_cc_exposure,aes(x=total_dur,fill=status,color=status))+geom_histogram(binwidth=20,alpha=0.3)+
  xlab('cumulative duration of exposure during follow-up (Days)')+scale_y_continuous(limits=c(0,400))
...
exposure_date<-exposure%>%group_by(nam)%>%summarise(first=min(dt_serv),
                                                 last=max(dt_serv))
...
model<-clogit(data=ramq_cc,status~exposure+sexe+diabete_combined+ckd_combined+hypertension_combined+pvd_combined+copd_combined+hyperlipidemia_combined+hf_combined+charlson+statin+strata(pair))
