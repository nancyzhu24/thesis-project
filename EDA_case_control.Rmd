...
```{r}
paste('Percent of cases sampled as control in Control group:',
round(100*sum(!is.na(ramq_cc$tcase)&ramq_cc$status==0)/sum(ramq_cc$status==0),2))
...
From the result, about 23% the controls became cases at a later time point after being sampled as control. Also, a high proportion of cases were sampled as control. Therefore, there is a high degree of similarity between case and control groups in this study.

#### Exposure pattern between case and control
```{r}
#load exposure dataset (subset of all prescription data where ahf code==481024)
exposure<-readRDS('C:/Users/nzhu/Desktop/Thesis project/Code for thesis/RData files/LTRA.RData')
...
#### Apply cut-off oof 90 days to exposure:
```{r}
...
##### Overall combined {-}
```{r}
variables<-c('tageind','tfu','sexe','diabete_combined','ckd_combined','hypertension_combined',
