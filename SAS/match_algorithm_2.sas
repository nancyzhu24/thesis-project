*this code is modified from Michelle's Nested case control sampling file;
*The purpose of the file is to create the nested case control cohort with density sampling approach matching on calender month of entry,length of follow-up;


libname data 'C:\Users\Nancy Zhu\OneDrive - McGill University\Code for thesis';

PROC IMPORT OUT= DATA.CASE_FINAL 
            DATAFILE= "C:\Users\Nancy Zhu\OneDrive - McGill University\Code for thesis\case_final_short.csv" 
            DBMS=CSV REPLACE;
     GETNAMES=YES;
     DATAROW=2; 
RUN;
...
PROC IMPORT OUT= DATA.COMPLETE_SET 
            DATAFILE= "C:\Users\Nancy Zhu\OneDrive - McGill University\Code for thesis\complete_set_short.csv" 
            DBMS=CSV REPLACE;
     GETNAMES=YES;
     DATAROW=2; 
RUN;
...
proc sql;
 ...
 select c.nam as caseid, t.nam as id,
		...
		c.year as cyear,t.year as tyear

 from case as c, controls as t
 where  c.year=t.year /*match on entry calendar year*/
       /* and (c.age+((c.exit_date-c.entry_date)/365.25)-1)<=(t.age+((c.exit_date-c.entry_date)/365.25))<=(c.age+((c.exit_date-c.entry_date)/365.25)+1)*/
